...
if(localStorage.getItem("lvlDis") == 1){
  if(lvl == 1){
...
}else if(localStorage.getItem("lvlDis") == 2){
  lvlFdw();
}
}else{
  cash = 15;
...
setInterval(function(){
  ...
  localStorage.setItem("shoppingOwned", shoppingOwned);
  localStorage.setItem("shoppingPrice", shoppingPrice);
  localStorage.setItem("lvlDis", lvlDisplay);
    },1000);
//Cash Update
setInterval(function(){
  if(cash < 1000){
...
function myFunction() {
  pause = 1;
  if(dev == 1){
    document.getElementById('devTools').innerHTML = "<p>Cash Slider</p><input type='range' id='devCash' min='0' max='100000000'><button onclick='saveCashDev()'>Save Cash</button><button onclick='exitDev()'>Exit Dev</button>"
  }
...
function exitDev(){
    ...
    document.getElementById('devTools').innerHTML = ""
}
...
function buyStuff(){
  const buyPopup = document.getElementById('buyStuff')
  buyPopup.showModal()
  const popup = document.getElementById("menu");
  popup.close()
}
